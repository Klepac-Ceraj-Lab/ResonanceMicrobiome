...
scatter!(fig1b, projection(kids_pco)[:,1] .* -1, projection(kids_pco)[:,2] .* -1,
...
has_age_pco = pcoa(kids_species)

fig1c = figure1[2, 1:2] = Axis(figure1, xlabel=mds_format(kids_pco, 1), ylabel="Age (years)")


scatter!(fig1b, projection(kids_pco)[:,1] .* -1, has_age_metadata.correctedAgeDays, label="Shannon diversity")
