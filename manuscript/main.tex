...
\caption{
    ...
    % (a) Ages of children when samples were collected for all children
    % or just children under 2 years of age (b).
    % (c) Total number of samples from children or samples at each age (d)
    % that were exclussively breastfed,
    % exclussively formula fed, or with a mixed diet.
    % These values refer to the diet \emph{when on liquid diet},
    % not necessarily current diet.
    % (d) Race and ethnicity of participants.
    % (e) Maternal SES as measured by the Hollingshead Scale.
    % (f) Age-normalized BMI z-score of children when samples were collected.
    % (g) Age-normalized cognitive score of children when samples were collected.
    % (f) Subjects with longitudinal timepoints by first collection.
    % "Infant": birth to 1 year, "early childhood": 1 year to 6 years,
    % "middle childhood": 6 years to 12 years, "adolescent": older than 12 years.
}
\label{fig:data}
\end{figure}
